...
import { pcAddCart, getGoodsDetail } from '../../api/api.js';
...
const getters = {
    ...
    get_goods_cart() {
        ...
        for (let i = 0; i < state.goods_model.length; i++) {
            if (state.goods_model[i].selected) {
                arr.push({
                    goodsNum: state.goods_model[i].num,
                    goodsId: state.goods_model[i].goodsId,
                    goodsSkuId: state.goods_model[i].goodsSkuId
            ...
            .then(
                response => {
                    ...
                    if (response.code == 'A0000') {
                        ...
                        commit(types.ADD_TO_SHOPING_CAR_FAILED, response.msg)
                    }
                }
            )
            .catch(
                e => {
                    commit(types.ADD_TO_SHOPING_CAR_FAILED, e)
                }
            )
...
const mutations = {
    ...
    // 添加到购物车失败
    [types.ADD_TO_SHOPING_CAR_FAILED](state, payload) {
    ...
    [types.GET_GOODS_DATA_SUCCESS](state, { response }) {
        ...
        for (let i = 0; i < length; i++) {
            model[i] = {
                ...
                goodsId: response.productsku[i].productId,
                goodsSkuId: response.productsku[i].id, // 规格sku 编码 
                selected: false,
                num: 0
            }
        }
